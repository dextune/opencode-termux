...
- Completed target version: `1.17.15`
...
This installer is **frozen** at `1.17.15`. There is no `VERSION` override and no plan to publish a generic installer that picks a "latest" tag. Why is captured in the next section.
...
- `opencode-ai@1.17.15` and `opencode-linux-arm64@1.17.15` are installed in the global npm prefix
- the platform package's binary exists and is executable
- `patchelf --print-interpreter` reports the Termux glibc loader path
- `opencode --version` reports `1.17.15`
...
3. Until those steps are completed for a new upstream version, the right way to install OpenCode on Termux is to stay on `1.17.15`.
